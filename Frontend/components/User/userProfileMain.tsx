...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedUserProfile) {
      try {
        ...
        // Reset form with stored user data
        reset({
  ...
  const handleSubmitForm: SubmitHandler<ProfileFormData> = async (data) => {
    const updatedUserData = {
      ...userData,
      address: `${data.street}, ${data.cityState}`,
          ...
          <form onSubmit={handleSubmit(handleSubmitForm)} className="mt-6 w-full">
            ...
            <div className="mb-3">
              ...
              <Controller
                name="username"
                control={control}
                rules={{ required: 'Username is required', pattern: /^[^\s]+(\s+[^\s]+)*$/ }}
                render={({ field }) => (
                  <input
                    type="text"
                    {...field}
            ...
            <button
              type="submit"
              className="w-full py-2 px-4 bg-pink-500 text-white rounded-lg hover:bg-pink-600 focus:outline-none focus:ring-2 focus:ring-pink-500"
            >
              Save Changes
            </button>
          </form>
